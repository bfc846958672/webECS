  ...
  render(system: ISystem, entityId: number) {
    ...
    for (const cmd of path.commands) {
      switch (cmd.type) {
        ...
        case "ellipse": ctx.ellipse(cmd.center[0], cmd.center[1], cmd.radiusX, cmd.radiusY, cmd.rotation ?? 0, cmd.start ?? 0, cmd.end ?? 2 * Math.PI, !!cmd.counterClockwise); break;
    ...
    for (const cmd of path.commands) {
      switch (cmd.type) {
        ...
        case "bezierCurveTo":
          for (let t = 0; t <= 1; t += 1 / steps) {
            const u = 1 - t;
            const x = u ** 3 * lastPos[0] + 3 * u ** 2 * t * cmd.cp1[0] + 3 * u * t ** 2 * cmd.cp2[0] + t ** 3 * cmd.end[0];
            const y = u ** 3 * lastPos[1] + 3 * u ** 2 * t * cmd.cp1[1] + 3 * u * t ** 2 * cmd.cp2[1] + t ** 3 * cmd.end[1];
            points.push(vec2.fromValues(x, y));
          }
          lastPos = [...cmd.end];
          break;
  ...
  hit(ecs: ECS, entityId: number, x: number, y: number): boolean {
    ...
    if (path.path2D) {
      ...
      if (hasClose) {
        ...
        return PathGraphics.isPointNearPath(path, local[0], local[1]);
      }
    }
  }
    ...
    for (const cmd of path.commands) {
      switch (cmd.type) {
        ...
        case "bezierCurveTo":
          for (let t = 0; t <= 1; t += 1 / steps) {
            const u = 1 - t;
            const px = u ** 3 * lastPos[0] + 3 * u ** 2 * t * cmd.cp1[0] + 3 * u * t ** 2 * cmd.cp2[0] + t ** 3 * cmd.end[0];
            const py = u ** 3 * lastPos[1] + 3 * u ** 2 * t * cmd.cp1[1] + 3 * u * t ** 2 * cmd.cp2[1] + t ** 3 * cmd.end[1];
            minDist = Math.min(minDist, Math.hypot(x - px, y - py));
          }
          lastPos = [...cmd.end];
          break;
