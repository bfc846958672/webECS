...
let polylineStrokeRoundProgram: Program | null = null;

function getOrCreatePolylineStrokeRoundProgram(gl: WebGL2RenderingContext): Program {
	if (polylineStrokeRoundProgram) return polylineStrokeRoundProgram;
	...
	polylineStrokeRoundProgram = new Program(gl, {
		vertex,
		fragment,
		transparent: true,
		cullFace: gl.NONE,
	...
	return polylineStrokeRoundProgram;
}

export function buildStrokeMeshRound(
	...
	// Fill joins (outer side) so round is visible and width is consistent
	...
	const program = getOrCreatePolylineStrokeRoundProgram(gl);
