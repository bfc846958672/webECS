...
import { drawBevelSectors } from './round';
...
export function buildStrokeMeshRound(
	...
	for (let i = startIndex; i < endIndex; i++) {
		...
		if (join.leftInner !== null) {
			bevels.push(points[p1], join.leftInner, rOffset.rs);
			bevels.push(points[p1], lOffset.re, join.leftInner);
			rounds.push(points[p1], rOffset.rs, lOffset.re);
			lOffset.le = join.leftInner;
			rOffset.ls = join.leftInner;
		} else if (join.rightInner !== null) {
			bevels.push(points[p1], join.rightInner, lOffset.le);
			bevels.push(points[p1],  rOffset.ls, join.rightInner);
			rounds.push(points[p1], lOffset.le, rOffset.ls);
	...
	// ================= 绘制圆角扇形 =================
	drawBevelSectors(gl, camera, aWorldMatrix, rounds, lineWidth, strokeColor, alpha);
	// ================= GPU 数据构建 =================
	let vertexCount = 0;
