 ...
 * @returns {vec3} out
 */
export function smoothLerp(out: vec3, a: vec3, b: vec3, decay: number, dt: number): vec3 {
 ...
 * Useful for rays.
 */
export function scaleRotateMat4(out: vec3, a: vec3, m: mat4): vec3 {
    let x = a[0],
        y = a[1],
        z = a[2];
    let w = m[3] * x + m[7] * y + m[11] * z + m[15];
...
export const angle = (function () {
    const tempA: vec3 = [0, 0, 0];
    const tempB: vec3 = [0, 0, 0];

    return function (a: vec3, b: vec3): number {
        copy(tempA, a);
        copy(tempB, b);
